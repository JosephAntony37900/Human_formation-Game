...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        print(f"Cargados {len(self.frames)} frames para Cristobal")
        
        if len(self.frames) <= 1:
            print("Solo se cargo 1 frame, no hay animacion")
        ...
        try:
            ...
            print(f"Error cargando imagen del arma: {e}")
    ...
    def load_frames(self, folder_path):
        ...
        try:
            ...
            print(f"Error cargando frames de Cristobal: {e}")
    ...
    def update(self, keys, min_allowed_x, max_allowed_x, min_allowed_y, max_allowed_y, level):
        # movimiento del jugador con A, W, S, D
        if keys[pygame.K_a]:
            if self.rect.left > min_allowed_x:
                self.rect.x -= self.speed
        if keys[pygame.K_d]:
            if self.rect.right < max_allowed_x:
                self.rect.x += self.speed
        if keys[pygame.K_w]:
        ...
        if keys[pygame.K_e]:
            ...
            print(f"Arma {'activada' if self.weapon_active else 'desactivada'}")
        ...
        if len(self.frames) > 1:
            if current_time - self.last_update >= self.frame_rate:
                ...
                print(f"Frame actual: {self.current_frame}")
